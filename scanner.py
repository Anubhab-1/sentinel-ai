...
async def check_port(target, port, service_name):
    ...
    try:
        reader, writer = await asyncio.wait_for(conn, timeout=1.0) # 1s timeout per port
        writer.close()
...
async def scan_ports_async(url):
    ...
    if not target:
         target = url.replace("https://", "").replace("http://", "").split('/')[0].split(':')[0]
...
async def analyze_https_enforcement_async(session, url):
    result = {"https_only": False, "redirects_to_https": False, "http_accessible": False}
    parsed = urlparse(url)
...
async def scan_website(url):
    findings = []
    metadata = {"reachable": True, "uses_js": False, "uses_forms": False, "https_context": {}, "header_consensus": {}}
    ...
    if found_ports:
        ...
        risky_ports = [p for p in found_ports if "HTTP (80)" not in p and "HTTPS (443)" not in p]
    ...
    async with aiohttp.ClientSession() as session:
        ...
        (resp_generic, text_generic), (resp_browser, text_browser), https_context = await asyncio.gather(
            task_generic, 
            task_browser, 
            task_https
        )
        ...
        valid_responses = []
        if resp_generic: valid_responses.append(resp_generic)
        if resp_browser: valid_responses.append(resp_browser)
        ...
        if text_browser:
            ...
            if soup.select('meta[name="generator"]'):
                ...
                if meta_gen: technologies.add(f"Generator: {meta_gen}")
            
            scripts = [s.get("src", "").lower() for s in soup.find_all("script") if s.get("src")]
            full_scripts = " ".join(scripts)
            
            if "jquery" in full_scripts: technologies.add("jQuery")
            if "react" in full_scripts or "_react" in full_scripts: technologies.add("React")
            if "vue" in full_scripts or "vue.js" in full_scripts: technologies.add("Vue.js")
            if "bootstrap" in full_scripts: technologies.add("Bootstrap")
            if "wp-content" in full_scripts or "wp-includes" in full_scripts: technologies.add("WordPress")
