...
def require_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if request.headers.get("X-CSRFToken") or "csrf_token" in request.form:
            ...
            log_audit_event(
                "AUTH_FAILURE", f"Invalid credentials from {request.remote_addr}"
            )
...
def not_found_error(error):
    ...
    return (
        render_template(
            "error.html",
            error_code=404,
            error_message="We couldn't find the page you're looking for.",
        ),
        404,
    )
...
def index():
    ...
    if request.method == "POST":
        ...
        if not is_safe_url(url):
            logger.warning(
                f"SSRF Attempt detected for URL: {url} from IP: {request.remote_addr}"
            )
            return (
                jsonify(
                    {"error": "Security Alert: Target is a forbidden internal address."}
                ),
                403,
            )
...
def explain():
    ...
    try:
        explanation = explain_finding(
            data.get("issue"), data.get("severity"), data.get("reasons", [])
        )
    except Exception as e:
...
def compare(url):
    ...
    scans = (
        Scan.query.filter_by(url=url).order_by(Scan.created_at.desc()).limit(2).all()
    )
    ...
    return render_template(
        "compare.html", url=url, latest=latest, previous=previous, diff=diff
    )
...
class PDFReport(FPDF):
    def header(self):
        ...
        self.cell(
            0,
            5,
            "Advanced Security Scanner & Audit Tool | RakshaNetra(TM)",
            0,
            1,
            "L",
            fill=False,
...
def download_pdf(scan_id):
    ...
    try:
        ...
        color = (
            (22, 163, 74)
            if score < 30
            else (220, 38, 38) if score > 70 else (217, 119, 6)
        )
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=sentinel_report_{scan_id}.pdf"
        )
        return response
